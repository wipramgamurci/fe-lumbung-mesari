<template>
    <div class="mb-8 flex justify-between items-center">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Pinjaman Saya</h1>
    ...
    <div v-if="pending" class="flex justify-center py-12">
      <UIcon name="i-heroicons-arrow-path" class="animate-spin w-12 h-12 text-primary-600" />
    </div>
    ...
    <UCard v-else-if="!loans?.length" class="text-center py-20 border-dashed">
      ...
      <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-white">Belum ada pinjaman</h3>
      <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Mulai dengan mengajukan pinjaman pertama Anda.</p>
...
const { data: loans, pending, error } = await useFetch<MemberLoan[]>('/api/users/me/loans');
