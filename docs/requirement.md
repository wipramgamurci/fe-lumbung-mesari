...
- **Week 1: Foundation & Authentication**
    - [ ] Basic Nuxt + Nuxt UI project setup.
    - [ ] Create **Login Page** (UI only).
    - [ ] Create **Register Page** (UI only, including file upload section).
    - [ ] Create an `auth` layout (an empty layout for login/register pages).
    - [ ] Connect the login page to the `/auth/login` API.

- **Week 2: Admin & Initial Dashboard**
    - [ ] Create the `default` layout (with a navigation sidebar).
    - [ ] Create the **Admin Page for Member Approval** (`/admin/members`).
    - [ ] Create the **Member Dashboard Page** (`/dashboard`), displaying dummy data initially.
