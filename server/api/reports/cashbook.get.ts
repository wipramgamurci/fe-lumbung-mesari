export default defineEventHandler(async (event) => {
  ...
  const month = monthStr ? parseInt(monthStr as string, 10) : defaultMonth;
  const year = yearStr ? parseInt(yearStr as string, 10) : defaultYear;
  ...
  try {
    const response = await $fetch<Blob>(
      `${config.public.apiBaseUrl}/api/reports/cashbook`,
      {
        method: "GET",
        query: {
          month,
    ...
    setResponseHeader(
      event,
      "Content-Disposition",
      `attachment; filename="buku-kas-${year}-${String(month).padStart(2, "0")}.xlsx"`,
    );
